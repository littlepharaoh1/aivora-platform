...
export default function App() {
 ...
 function updateDecision(id:string, value:Decision){
  setRecords(old =>
    old.map(r => r.id === id ? { ...r, decision:value } : r)
  );
...
{tab==="qc" && <section className="panel"><h2>QC Audio Analyzer</h2><p>QC Panel Active</p><div className="actionRow"><button className="success" onClick={() => selected && updateDecision(selected.id,"Approved")}>Approve</button><button className="warn" onClick={() => selected && updateDecision(selected.id,"Review")}>Review</button><button className="danger" onClick={() => selected && updateDecision(selected.id,"Rejected")}>Reject</button></div><p>Selected: {selected?.fileName || "No file selected"}</p></section>}      {tab==="contributors" && <section className="panel"><h2>Contributor Management</h2><table><thead><tr><th>ID</th><th>Name</th><th>Submitted</th><th>Approved</th><th>Review</th><th>Rejected</th></tr></thead><tbody>{["D0001","D0002","D0003","D0004"].map((id,i)=><tr key={id}><td>{id}</td><td>Contributor {String(i+1).padStart(3,"0")}</td><td>0</td><td>0</td><td>0</td><td>0</td></tr>)}</tbody></table></section>}
