...
const [role, setRole] = useState<string | null>("Admin");
  ...
  const { can, allowedTabs, role: userRole } = usePermissions();
useEffect(() => {
...
const mergeRoomFiles = async () => {
  ...
  if (!filesA.length || !filesB.length) {
    alert("Please upload multiple clips for Speaker A and Speaker B.");
    return;
  }
  ...
  const targetLength = sr * targetSeconds;
  ...
  const decodedA = await Promise.all(filesA.map(decodeFile));
  const decodedB = await Promise.all(filesB.map(decodeFile));

  const detectSegments = (buf: AudioBuffer) => {
    const data = buf.getChannelData(0);
    const th = 0.014;
    const hold = Math.floor(buf.sampleRate * 0.22);
    const minLen = Math.floor(buf.sampleRate * 0.45);
    ...
    for (let i = 0; i < data.length; i++) {
      if (Math.abs(data[i]) > th) {
        if (!active) {
          active = true;
          start = Math.max(0, i - Math.floor(buf.sampleRate * 0.05));
        }
        silent = 0;
      } else if (active) {
        silent++;
        if (silent >= hold) {
          const end = Math.min(data.length - 1, i);
    ...
    if (active && data.length - start > minLen) {
      segs.push({ start, end: data.length - 1 });
    }

    return segs.length ? segs : [{ start: 0, end: data.length - 1 }];
  };

  const poolA = decodedA.flatMap(({ file, buf }) =>
    detectSegments(buf).map((seg) => ({ speaker: "A" as const, file, buf, ...seg }))
  );

  const poolB = decodedB.flatMap(({ file, buf }) =>
    detectSegments(buf).map((seg) => ({ speaker: "B" as const, file, buf, ...seg }))
  );

  if (!poolA.length || !poolB.length) {
    alert("No valid speech segments detected.");
    return;
  }

  const out = ctx.createBuffer(2, targetLength, sr);
  ...
  while (cursor < targetLength && safety < 10000) {
    safety++;

    const gap = Math.floor(sr * (0.18 + Math.random() * 0.35));
    const item = turnA ? poolA[ia++ % poolA.length] : poolB[ib++ % poolB.length];
    const src = item.buf.getChannelData(0);
    const dur = item.end - item.start + 1;
    const fade = Math.min(Math.floor(item.buf.sampleRate * 0.025), Math.floor(dur / 4));

    for (let i = 0; i < dur && cursor + i < targetLength; i++) {
      let g = 1;
      if (i < fade) g = i / Math.max(1, fade);
      if (i > dur - fade) g = Math.min(g, (dur - i) / Math.max(1, fade));

      const sample = Math.max(-0.95, Math.min(0.95, src[item.start + i] * 0.92 * g));

      if (item.speaker === "A") {
        L[cursor + i] += sample;
        R[cursor + i] += sample * 0.10;
      } else {
        R[cursor + i] += sample;
        L[cursor + i] += sample * 0.10;
      }
    }
  ...
  link.href = url;
  link.download = "conversation_room_v8_30min_multi_clip.wav";
  link.click();
...
{roomFileSummary}
    ...
    <input
      ...
      hidden
      onChange={(e) =>
        (() => { const fs = Array.from(e.target.files || []); setRoomFilesA(fs); setRoomFileA(fs[0] || null); })()
      }
    />
    ...
    <input
      ...
      hidden
      onChange={(e) =>
        (() => { const fs = Array.from(e.target.files || []); setRoomFilesB(fs); setRoomFileB(fs[0] || null); })()
      }
    />
  </section>
)}
...
{tab === "audition" && <AivoraAuditionWorkstation />}
