 ...
 function downloadText(name: string, text: string, type = "text/plain") {
  ...
  return <div className={`app ${theme}`}>
    ...
    <main>
      ...
      {tab==="control" && <section className="panel"><div className="panelHead"><h2>Control Center</h2><p>Dynamic Project Configuration</p></div><div className="ruleGrid"><div>Project: {config.projectName}</div><div>Client: {config.clientName}</div><div>Language: {config.language}</div><div>Locale: {config.locale}</div><div>Total Files: {config.totalFiles}</div><div>Sample Rate: {config.sampleRate}</div><div>Bit Depth: {config.bitDepth}</div><div>Channels: {config.channels}</div><div>Noise Limit: {config.maxNoiseDb}</div><div>Deadline: {config.deadline}</div></div></section>}
